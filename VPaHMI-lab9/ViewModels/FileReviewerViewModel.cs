...
{
    internal class FileReviewerViewModel : ReactiveObject
    {
        ...
        public FileReviewerViewModel()
        {
            var assetLoader = AvaloniaLocator.Current.GetService<IAssetLoader>();

            using (var fileStream = assetLoader.Open(new Uri("avares://VPaHMI-lab9/Assets/file.png")))
            using (var folderStream = assetLoader.Open(new Uri("avares://VPaHMI-lab9/Assets/folder.png")))
            using (var folderOpenStream = assetLoader.Open(new Uri("avares://VPaHMI-lab9/Assets/folder-open.png")))
            {
        ...
        private void SetSelectedPath(string? value)
        {
            ...
            else if (Directory.Exists(path)) { d = new DirectoryInfo(path); }
            ...
            if (components.Count > 0)
            {
                ...
                if (driveIndex >= 0) SelectedDrive = Drives[driveIndex];
        ...
        private void SelectionChanged(object? sender, TreeSelectionModelSelectionChangedEventArgs<FileTreeNodeModel> e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (jpgs.Length > 1) { AreButtonsActive = true; }
                }
                catch (Exception)
                {
